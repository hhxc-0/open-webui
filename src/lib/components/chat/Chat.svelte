<script lang="ts">
	...
	let pendingWebSearchMode: '' | 'external' | 'native' = '';
	...
	const handleWebSearchToggle = (mode: '' | 'external' | 'native') => {
		if (mode && $config?.features?.enable_web_search_confirmation && !webSearchConfirmed) {
			pendingWebSearchMode = mode;
			webSearchMode = '';
	...
	const resetWebSearchConfirmation = () => {
		...
		pendingWebSearchPrompt = null;
		pendingWebSearchMode = '';
		showWebSearchConfirm = false;
	};

	$: if (!webSearchActive && !pendingWebSearchMode && pendingWebSearchPrompt === null) {
			...
			if (model) {
				...
				if (model?.info?.meta?.defaultFeatureIds) {
					...
					if (
						...
						webSearchMode = model.info.meta.defaultFeatureIds.includes('native_web_search')
							? 'native'
	...
	const confirmWebSearch = async () => {
		...
		} else if (pendingWebSearchMode) {
			webSearchMode = pendingWebSearchMode;
			pendingWebSearchMode = '';
		}
	};
...
<WebSearchConfirmDialog
	...
	on:cancel={() => {
		if (pendingWebSearchPrompt === null) webSearchMode = '';
		pendingWebSearchPrompt = null;
		pendingWebSearchMode = '';
	}}
/>
