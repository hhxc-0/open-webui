...
async def prepare_chat_request(request: Request, form_data: dict, user):
    ...
    if isinstance(message_ids, list):
        ...
        message_ids = [{'model_id': key, 'message_id': value} for key, value in message_ids.items()]
        form_data.pop('id', None)
    else:
...
async def chat_completion(
    ...
    try:
        ...
        chat_variables = metadata['chat_variables']
        is_new_chat = metadata['is_new_chat']

        initial_title_generation = None
        if metadata.get('is_new_chat', False) and tasks and TASKS.TITLE_GENERATION in tasks:
